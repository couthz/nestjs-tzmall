...
export const paginate = async <E extends ObjectLiteral>(
    ...
    const size = isNil(options.size) || options.size < 1 ? 1 : options.size;
    const current = isNil(options.current) || options.current < 1 ? 1 : options.current;
    const start = current >= 1 ? current - 1 : 0;
    const totalItems = await qb.getCount();
    qb.take(size).skip(start * size);
    const items = await qb.getMany();
    const totalPages =
        totalItems % size === 0
            ? Math.floor(totalItems / size)
            : Math.floor(totalItems / size) + 1;
    const remainder = totalItems % size !== 0 ? totalItems % size : size;
    const itemCount = current < totalPages ? size : remainder;
    return {
        items,
        meta: {
            totalItems,
...
export function treePaginate<E extends ObjectLiteral>(
    ...
    const totalRst = totalItems / size;
    const totalPages =
        totalRst > Math.floor(totalRst) ? Math.floor(totalRst) + 1 : Math.floor(totalRst);
    let itemCount = 0;
    if (current <= totalPages) {
        itemCount = current === totalPages ? totalItems - (totalPages - 1) * size : size;
